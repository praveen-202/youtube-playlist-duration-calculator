...
// Service to fetch and calculate YouTube playlist duration
@Service
public class YoutubeService {

    // Injected YouTube API key from application.properties
    ...
    // Fetches playlist data and calculates duration and adjusted times
    public PlaylistResponse fetchPlaylistDuration(String playlistUrl) {
        String playlistId = extractPlaylistId(playlistUrl);
        List<String> videoIds = new ArrayList<>();

        String nextPageToken = "";
        // Paginate through all videos in the playlist (max 50 per request)
        do {
        ...
        // Fetch duration for each batch of up to 50 videos
    ...
    // Extracts playlist ID from YouTube URL using regex
    ...
    // Returns a map of playback speeds and corresponding adjusted durations
    private Map<String, String> getAdjustedTimes(long totalSeconds) {
        Map<String, String> speeds = new LinkedHashMap<>();
        double[] factors = {1.0, 1.25, 1.5, 1.75, 2.0};
